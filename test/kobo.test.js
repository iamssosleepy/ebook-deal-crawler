...
test('loads only the requested week from validated Kobo JSONL', async t => {
  ...
  const rows = [
    ...
    },
    {
      ...
      date: '2026-08-28',
      title: '《缺少來源的書》',
      tw_url: 'https://www.kobo.com/tw/zh/ebook/missing-source'
    }
  ];
  ...
  assert.equal(deals[0].sourcePage, 'https://www.kobo.com/zh/blog/weekly-dd99-2026-w35/');
});

test('rejects Kobo fallback rows without an official source reference', async t => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'kobo-jsonl-'));
  t.after(() => fs.rm(directory, { recursive: true, force: true }));
  const file = path.join(directory, 'books.jsonl');
  const row = {
    ...
    date: '2026-08-27',
    title: '《無來源書籍》',
    tw_url: 'https://www.kobo.com/tw/zh/ebook/no-source'
  };
